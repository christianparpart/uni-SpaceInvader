...
std::vector<sgfx::point> convex_hull(std::vector<sgfx::point> points)
{
	...
	const auto min_it=std::min_element(std::begin(points),std::end(points),[](const auto& lhs, const auto& rhs)
	{
	...
	{
		while(ret_val.size()>2 && compute_turn(ret_val[ret_val.size()-2],ret_val[ret_val.size()-1],p)==direction::clockwise)
			ret_val.pop_back();
...
void draw_outline(sgfx::canvas_view target, const sgfx::rectangle& rect, sgfx::color::rgb_color col)
{
	...
	sgfx::line(target,rect.top_left+sgfx::vec{rect.size.w-1,0}, rect.top_left+sgfx::vec{rect.size.w-1,rect.size.h},col);
	sgfx::line(target,rect.top_left+sgfx::vec{0,rect.size.h-1}, rect.top_left+sgfx::vec{rect.size.w,rect.size.h-1},col);
}
...
int main(int argc, char* argv[])
{
	...
	std::generate_n(std::back_inserter(polys),100,[&]()
	{
		...
		const auto offset=sgfx::vec{random_uniform_int(main_window.width()),random_uniform_int(main_window.height())};
