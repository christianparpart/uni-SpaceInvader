...
class quadtree<T, max_depth>::node_iterator {
  public:
    node_iterator(sgfx::rectangle area, const quadtree<T, max_depth>::node_t* node) { /*do something*/ }
    ...
    struct node_ref {
        ...
        { /*return the area coverd by the current node*/
        }
        auto& elements() const
        { /*return something that can be used to iterate over the elements contained in the current node*/
        }
  ...
  private:
    ...
    friend bool operator==(const node_iterator& iter, iteration_sentinel) noexcept
    {
        ...
        // In its simplest version, this provides a simple way to check for the end without having to compare
        // with some invalid value: if the traversal stack/queue is empty, the end was reached
    }
...
class quadtree<T, max_depth>::iterator {
  public:
    ...
    {
        // construct an iterator useful for iterating over all elements of node_iter and all elements in
        // children thereof
    }
...
typename quadtree<T, max_depth>::iterator quadtree<T, max_depth>::insert(const T& value)
{
    ...
    // Recursivley subdivide and extend the tree until we find the smallest subarea into which value fits and
    // insert into this node Return an iterator to the newly inserted element (This involves computing an AABB
    //for value. Provide a customization point of your choice to do so)
}
