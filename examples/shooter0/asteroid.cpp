...
asteroid::asteroid(game_proxy proxy, const std::string& filename, sgfx::point pos):
	autoregister_collider{proxy},
	img_{sgfx::load_rle(filename)},
	pos_{pos}
{}
...
game_object::status asteroid::update(game_proxy proxy, std::chrono::milliseconds delta)
{
	...
	if(
		(pos_.x-center_offset.x>area.top_left.x+area.size.w && vel_.x>0) ||
