...
class spawn_point final: public game_object
{
	...
	spawn_point(sgfx::rectangle area, spawn_function_type spawn_fun, std::chrono::milliseconds delay):
		area_{area},
		spawn_fun_{spawn_fun},
		delay_{delay},
		time_remaining_{delay}
	{}
	...
	game_object::status update(game_proxy proxy, std::chrono::milliseconds delta) override
	{
		...
		{
			const auto pos=sgfx::point{area_.top_left.x+random_uniform_int(area_.size.w),area_.top_left.y+random_uniform_int(area_.size.h)};
...
template <typename spawn_function_type>
spawn_point(sgfx::rectangle, spawn_function_type, std::chrono::milliseconds) -> spawn_point<spawn_function_type>;
