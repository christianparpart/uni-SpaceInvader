...
player::player(game_proxy proxy, const std::string& name, const key_config& keys):
	autoregister_collider{proxy},
	imgs_{sgfx::load_rle(name+".rle"),sgfx::load_rle(name+"_left.rle"),sgfx::load_rle(name+"_right.rle")},
	keys_{keys},
	pos_{proxy.area().top_left+sgfx::vec{proxy.area().size.w/2,proxy.area().size.h}}
{
}
...
game_object::status player::update(game_proxy proxy, std::chrono::milliseconds delta)
{
	...
	pos_.x=std::clamp(pos_.x,area.top_left.x+center_offset.x,area.top_left.x+area.size.w-center_offset.x);
	pos_.y=std::clamp(pos_.y,area.top_left.y+center_offset.y,area.top_left.y+area.size.h-center_offset.y);
