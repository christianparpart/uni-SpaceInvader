...
void sgfx::draw(canvas_view target, const canvas& img, point top_left)
{
    ...
    for (int line = 0; line < line_num; ++line)
    {
        ...
        std::copy_n(&img.pixels()[src_y * img.width() + src_x], line_length,
                    &target.pixels()[dest_y * target.width() + dest_x]);
    }
}
...
void sgfx::draw(canvas_view target, const canvas& img, point top_left, color::rgb_color color_key)
{
    ...
    for (int line = 0; line < line_num; ++line)
    {
        ...
        std::transform(begin, end, target_begin, target_begin,
                       [&](auto c_dest, auto c_target) { return c_dest == color_key ? c_target : c_dest; });
    }
}
