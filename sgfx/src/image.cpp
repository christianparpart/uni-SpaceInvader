...
{
	if(top_left.y>=target.height() || top_left.y+source.height_<0) return;
	if(top_left.x>=target.width() || top_left.x+source.width_<0) return;
	
	const auto y_start=std::max(0,-top_left.y);
	const auto y_end=top_left.y+source.height_<target.height()?source.height_:(target.height()-top_left.y);
	
	std::size_t run_offset=std::accumulate(std::begin(source.line_runs),std::begin(source.line_runs)+y_start,0);
	for(std::uint16_t y=y_start;y<y_end;++y)
	{
		...
		{
			if(x_offset>=target.width()) break;
			
			if(x_offset+source.runs[run_offset+r].num>=0)
			{
				const auto x_start=std::max(x_offset,0);
				const auto x_end=std::min(x_offset+source.runs[run_offset+r].num,static_cast<int>(target.width()));
				std::fill(&target.pixels()[(y_offset+y)*target.width()+x_start],&target.pixels()[(y_offset+y)*target.width()+x_end],source.runs[run_offset+r].color);
			}
...
void sgfx::draw(canvas_view target, const rle_image& source, point top_left, color::rgb_color colorkey)
{
	...
	for(std::uint16_t y=y_start;y<y_end;++y)
	{
		...
		for(std::uint16_t r=0;r<source.line_runs[y];++r)
		{
			...
			if(x_offset+source.runs[run_offset+r].num>=0 && source.runs[run_offset+r].color!=colorkey)
			{
				const auto x_start=std::max(x_offset,0);
				const auto x_end=std::min(x_offset+source.runs[run_offset+r].num,static_cast<int>(target.width()));
				std::fill(&target.pixels()[(y_offset+y)*target.width()+x_start],&target.pixels()[(y_offset+y)*target.width()+x_end],source.runs[run_offset+r].color);
